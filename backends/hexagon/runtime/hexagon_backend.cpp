...
// Whether this (space, index) was given a slot in the persistent state block.
// The packed attention cache and the KV cache itself live there; everything
...
Error ResizeDynamicDelegate(HexagonDelegate* delegate, int64_t length) {
  ...
  for (uint32_t i = 0; i < delegate->inputs.size(); i++) {
    // A persistent cache is not in the per-call scratch, so it takes no room in
    // the input budget and is never copied in.
    if (IsStateRef(*delegate, HexagonTensorSpace::kInput, i)) {
      continue;
    }
  ...
  for (uint32_t i = 0; i < delegate->outputs.size(); i++) {
    // A persistent cache result is the same slot its input names, so it takes
    // no room in the output budget and is never copied back to the caller.
    if (IsStateRef(*delegate, HexagonTensorSpace::kOutput, i)) {
      continue;
    }
    auto* layout = const_cast<HexagonDelegate::RuntimeLayout*>(
...
Result<DelegateHandle*> HexagonBackend::init(
  ...
  //
  // The KV cache itself is the same story: the graph advances it with
  // update_cache and reads it back whole on every later step, and its method
  // input and output are the two ends of one buffer. Left in the pooled scratch
  ...
  auto add_state = [&](HexagonTensorSpace space,
    ...
    delegate->state_slots.push_back({space, index, offset, size});
    state_bytes = offset + size;
  };
  for (uint32_t i = 0; i < header->n_ops; i++) {
    ...
    if (op.type == kFlashAttnOp && op.n_inputs >= 6) {
      for (uint32_t j = 4; j <= 5; j++) {
        ...
        if (ref.space == static_cast<uint32_t>(HexagonTensorSpace::kActivation)) {
          add_state(HexagonTensorSpace::kActivation, ref.index, ref.size);
        }
      }
    }
    // The cache advance is a three-input, patched blit whose second input is
    // the method's cache slot and whose result is the method's updated-cache
    // slot. `params[2]` is the blit's source count, which pins the three-input
    // form apart from the two-input slice blits that also carry a patch.
    if (kv_state && op.type == kRasterBlitOp && op.n_inputs == 3 &&
        op.n_outputs == 1 && op.n_params >= 3 && op.params[2] == 3 &&
        op.patch_param != kNoOpPatch &&
        op.inputs[1].space == static_cast<uint32_t>(HexagonTensorSpace::kInput) &&
        op.outputs[0].space == static_cast<uint32_t>(HexagonTensorSpace::kOutput) &&
        op.inputs[1].size == op.outputs[0].size) {
      const auto& in = op.inputs[1];
      const auto& out = op.outputs[0];
      if (!IsStateRef(*delegate, HexagonTensorSpace::kInput, in.index) &&
          !IsStateRef(*delegate, HexagonTensorSpace::kOutput, out.index)) {
        const size_t offset = AlignUp(state_bytes, kHexagonAlignment);
        state_bytes = offset + out.size;
        delegate->state_slots.push_back(
            {HexagonTensorSpace::kInput, in.index, offset, out.size});
        delegate->state_slots.push_back(
            {HexagonTensorSpace::kOutput, out.index, offset, out.size});
        std::fprintf(
            stderr,
            "[hexagon] d%d: KV cache in %u / out %u -> state slot %zu (%zu bytes)\n",
            delegate->index,
            in.index,
            out.index,
            delegate->state_slots.size() - 2,
            out.size);
      }
    }
  }
  if (state_bytes > 0) {
    ...
    if (!state.ok()) {
      ET_LOG(Error, "hexagon: persistent state allocation failed");
      return state.error();
    }
...
Error HexagonBackend::execute(
  ...
  for (size_t i = 0; i < delegate->inputs.size(); i++) {
    ...
    // The cache lives in the persistent block and the graph advances it in
    // place, so the caller's copy is a stale snapshot the commands must not
    // read. Its input slot is the state slot, already where the graph left it.
    if (IsStateRef(*delegate, HexagonTensorSpace::kInput, i)) {
      continue;
    }
  ...
  for (size_t i = 0; i < delegate->outputs.size(); i++) {
    ...
    // The updated cache is the same persistent slot its input named; the graph
    // already advanced it in place, so there is nothing to hand back.
    if (IsStateRef(*delegate, HexagonTensorSpace::kOutput, i)) {
      continue;
    }
