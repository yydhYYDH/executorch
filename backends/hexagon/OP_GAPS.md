...
`htp_command.h:32-83` declares 49 op types. 44 have a `case` in
`execute_command.cc`, and the host side emits 22 of them. The other 21 are
...
Of the 49, 44 have a dispatcher case: the 22 the host emits, the 21 above, and
...
`DSP_OP_ARGMAX_FP16`(47) with `argmax`/`argmin`. The counts above read 22
dispatcher cases and 21 rows where `b7183d9` read 19 and 23: the same table with
two fewer rows and the command set with three more. Each is worth reading in §3, and
the ones that carry a qualification are there: the prefill entry reaches the int4
weight and not the int8 one, `where` is placed while the comparison that
produces its condition is not, and the arg reduction walks the last axis only.
...
kernel rather than an emitter: a one-byte bool output for the comparisons.
