...
| `pool_spec`: window and counting | any static kernel and stride, `pad >= 0`, the output size floor division gives, every window inside its row | `dilation != (1, 1)`, `ceil_mode` (windows past the input, which this geometry does not describe), `divisor_override` | `:2125`, `:2136`, `:2142` |
