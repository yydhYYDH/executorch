...
| The weight-only quantized matmul needs `K % 64 == 0` and `N % 32 == 0`, and then either a single activation row (the GEMV entries) or an int4 weight (the prefill entry); the int4 prefill also needs `K <= 12672` while `M <= 32` (`PREFILL_M32_MAX_M` / `PREFILL_M32_MAX_K` in `hexagon_ops.py`). The measured VLA skel kept one 32-byte descriptor per `K/32` activation tile on the stack (`matmul_ops.cc:28`, `matmul_q4fp16_mle32.c:528`), while the baseline source now heap-allocates that array. The host ceiling remains because deployment does not encode source provenance: the heap skel passed `K = 25216` for `M = 2, 4, 32`, but the stale VLA skel still aborted with `0x8000040d` | a w8a16 matmul above one row, a batch axis, a dynamic (`SymInt`) `M`, and an int4 prefill with `M <= 32` at `K > 12672` -- widening the Q4 ceiling needs a matched rebuild and broader device evidence; the int8 prefill still needs a packer for command 42 because its kernel tile order is not written by this tree |
