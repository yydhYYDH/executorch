...
| `aten.prod.default` | A running product is a different walk from the four reductions that exist (sum, maximum, mean, minimum -- minimum arrived with merge 13), and the log/sum/exp substitute is inexact as well: sixteen values in [2, 3] already give 2**16, which is an infinity in fp16. `aten.cumsum.default` moved off this row: a prefix scan is not a reduction the kernel has but a product the kernel does have, against a host mask, so it is wired through `et_hexagon.cumsum.default` |
| `aten.var.correction` | A second moment is a different walk from the four reductions that exist, and no composition of the commands that do is a substitute: the two-pass form -- mean, subtract, square, mean, scale -- is five commands of types the table already has and reproduces the *fp16 two-pass* bit for bit, which is a different claim. Measured against `torch.var` on fp16 it is 7.5e-04 relative on centred data and 4.0e-03 with a mean of 200, because the square is materialised in fp16 and torch's accumulator is fp32; and seven zeros with one 400, whose variance is 20000 and perfectly representable, needs (400 - 50)**2 = 122500 as an intermediate, which is an infinity. The reduction kernel accumulates in fp32 but reads fp16, so no command arrangement avoids the square, and the export cannot bound the deviation of a run-time activation. `test_zero_cost_ops.py` pins the composition's command stream and both numbers |
...
`aten.clamp.Tensor` left this table for the reason the last paragraph gives, and
its composition is the shortest one here: the clamp entry point cannot hold a
tensor, so a bound that is an operand becomes `min(max(x, lo), hi)` -- one
...
- **`aten.expm1`**: measured and **not** wired, and it is the counterpart to the
  ...
  `aten.elu.default` now carries the same cancellation on purpose, inside
  `min((exp(x) - 1) * alpha * scale, 0)`, and it is worth being explicit about why
  that is a different decision. For `expm1` the cancellation *is* the op: the value
  near zero is the only value, and a relative error of 1.0 there makes the entry a
  constant over the interval it exists for. For the elu the cancelled band is one
  side of a branch that saturates at `-alpha * scale`, so the error is bounded in
  absolute terms at 2.4e-4 -- 0.2% of the branch's range -- while the relative error
  still reaches 1.0 below an `abs(x)` of about 3e-4. Both halves are measured in
...
| One contiguous `[outside][reduce][inside]` span per reduction | `mean.dim`, `sum.dim_IntList`, `amax` and `amin` with non-adjacent reduced dims -- measured, all four refuse, not the three this row used to name. The span's *width* is not a limit at all: every `reduce` and `inside` extent from 1 to 3300 is admitted, the ragged ones included, because `htp_ops_reduction` finishes a partial vector in fp32 (`htp_ops_reduce_fp16_scalar_range`) rather than truncating it | a strided reduction, which is the same kernel the softmax row below needs |
