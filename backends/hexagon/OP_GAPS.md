...
| The weight-only quantized matmul needs `K % 64 == 0` and `N % 32 == 0`, and then either a single activation row (the GEMV entries) or an int4/int8 weight (the prefill entry) | a batch axis or a dynamic (`SymInt`) `M` | general prefill support for those additional geometries; the int8 packer and fp16 scale tail are present for the supported M>1 path |
