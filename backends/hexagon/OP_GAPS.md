...
| The weight-only quantized matmul needs `K % 64 == 0` and `N % 32 == 0`, and then either a single activation row (the GEMV entries) or an int4 weight (the Q4 prefill entry); the Q4 prefill also needs `K <= 12672` while `M <= 32` (`PREFILL_M32_MAX_M` / `PREFILL_M32_MAX_K` in `hexagon_ops.py`). The current small-M DSP source heap-allocates the per-tile descriptors at `matmul_q4fp16_mle32.c:443-447` and frees them at `:691`; the historical VLA used the same count on the stack. The host ceiling remains because deployment does not encode source provenance: a measurement-only OnePlus 13 A/B found the heap skel passed `K = 25216` for `M = 2, 4, 32`, while the stale VLA skel aborted with `0x8000040d` | a w8a16 matmul above one row, a batch axis, a dynamic (`SymInt`) `M`, and an int4 prefill with `M <= 32` at `K > 12672` -- widening the Q4 ceiling needs a matched rebuild and broader device evidence; the int8 prefill still needs a packer for command 42 because its kernel tile order is not written by this tree |
...
1. **Quantized prefill (`M > 1`) for the int8 weight** -- the int4 half landed and
   ...
   §4 also carries the conservative Q4 ceiling the wired int4 entry now holds
   (`K <= 12672` below the dispatcher's `M <= 32` split): that one is a refusal
   past a measured bound, not a missing packer. The heap-backed source passes
   larger K on a matched device run, but the host keeps the ceiling because a
   stale VLA skel can still be deployed; widening it needs provenance and a
   broader matrix.
