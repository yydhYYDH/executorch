...
| `aten.convolution.default with transposed=True` and a dilation above 1, or a group count other than 1 | A transposed convolution *is* a window walk once its input is interleaved with zeros: `conv_transpose(x, w, s, p, op) == conv2d(zero_insert(x, s, op), flip(w).transpose(ic, oc), k - 1 - p)`, which `conv_spec` emits on the same two kernels. The undilated, one-group case is therefore supported. A dilated transposed window is a different identity. A group count needs no channel mapping on the kernel at all once the host partitions: neither walk takes a group field, so the grouped form is `groups` dense walks and the grouped transposed form would be the same partition behind the zero-insert. The plain grouped form is a supported row now (`_emit_grouped_convolution` in `hexagon_ops.py`, `test_grouped_conv.py`); the grouped *transposed* one is not in this branch, because the identity above has to hold per group and the zero-insert's per-group record is the same shape of work this row is about |
