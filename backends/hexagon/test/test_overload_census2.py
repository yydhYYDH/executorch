...
_QUANTIZED_ROWS = [
    ...
    (
        # The pair is not in the graph here at all. The quantizer asks the
        # emitters' question before it annotates, and there is no w8a16 prefill
        # entry, so this shape keeps the fp16 matmul it had rather than carry a
        # dequantize the partitioner would refuse -- a refusal `to_executorch`
        # cannot recover from, since `quantized_decomposed` has no out variant.
        # The row used to record the pair as refused.
