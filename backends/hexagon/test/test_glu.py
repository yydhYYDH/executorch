...
def test_the_glu_on_the_host_interpreter_answers_torch(channels, length):
    ...
    This test used to measure rounding alone, on the premise that the
    interpreter's sigmoid was the exact fp32 logistic. It no longer is: a later
    branch gave `blob_interpreter.py` the kernel's real companded16 PWL chords
    for the vector path, which is a more faithful host model of the device and
    not the same arithmetic. So the disagreement measured here is the PWL band
    the sibling test below quantifies against the kernel's own scalar path, and
    the tolerance says so. The four lengths still bracket the kernel's 64-element
    grain -- 1536 a whole number of walks, 1584 and 112 each leaving a scalar
    tail, 48 never taking the walk -- so the geometries that would behave
    differently on device are all represented.
    """
    torch.manual_seed(0)
    ...
    # The vector path walks 64 fp16 at a time down to the PWL chords while the
    # scalar tail keeps the exact logistic, so this is a piecewise-linear error
    # and not two roundings. The band is the one the sibling test measures, and
    # the second assertion states it in fp16 steps at the product's own scale so
    # a change in the output's magnitude cannot silently loosen it.
    assert np.abs(got.astype(np.float64) - want.astype(np.float64)).max() <= (
        7.9e-3
    ), f"{channels}x{length}"
    steps = np.abs(
        got.astype(np.float16).view(np.uint16).astype(np.int64)
        - want.astype(np.float16).view(np.uint16).astype(np.int64)
    )
    assert steps.max() <= 64, f"{channels}x{length} in fp16 steps: {steps.max()}"
...
def test_the_conformer_conv_module_lowers_as_one_delegate_with_both_glu_in_it():
    ...
    # The pointwise convolutions used to lower to the im2col kernel (type 12).
    # A 1x1 convolution is the case where the im2col regions are the identity,
    # and the branch that named the kernel's own 1x1 command (merge 9) gives those
    # two convolutions their own DSP_OP_CONV1X1_DIRECT_FP16 (17) instead. Same
    # two commands at the same two positions, so the GLU windows above are
    # untouched; only which command carries the convolution moved.
    conv1x1 = [command for command in commands if command.type == 17]
    assert len(conv1x1) == 2, types
    assert not [command for command in commands if command.type == 12], types
    for command in conv1x1:
    ...
    # reduction order. The branch measured 5.86e-3 against the im2col path, but
    # the pointwise convolutions here lower to the 1x1 direct command, and that
    # path measures 7.8125e-3 -- exactly 2.0 fp16 steps at this output's scale,
    # which is the check that matters: the tolerance is stated in steps as well
    # as absolute, so a change in the output's magnitude cannot silently loosen
    # it. 7.8125e-3 is the absolute form of 2 steps at 5.402.
    assert error <= 7.9e-3, f"the block's error: {error}"
