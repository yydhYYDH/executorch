...
def test_a_one_axis_convolution_and_its_four_dimensional_form_agree_on_groups():
    """The reading decides the axis, not the group rule.

    16 groups over 64 input channels is a grouped convolution, and the host
    lowers one to a dense command per group, so both spellings reach the DSP.
    The 4-D graph of the same groups is the control: the one-axis reading is
    the 4-D one-column window, so the two must agree, and they do -- on
    delegating. Both were refusals until the per-group partition landed, and
    this row was written when they were.
    """
