...
def _emit_convolution(node: torch.fx.Node, ctx) -> TensorRef:
    ...
    if spec.depthwise:
        ...
        op_index = ctx.emit(
            node,
            Op(
                # 17 names the function's 1x1 activation fill, which this
                # geometry takes; 12 is the same function on its general window
                # walk. conv_1x1_direct_applies is the host transcription of the
                # C's own selection, so the stream says which fill runs.
                type=(
