...
tile order nothing here packs. The quantizer asks these same conditions, through
`weight_only_matmul_fits` and the bias check, before it annotates anything, and
an `M > 1` w8a16 matmul is therefore never annotated at all.
