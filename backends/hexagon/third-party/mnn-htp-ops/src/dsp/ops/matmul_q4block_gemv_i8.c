...
// scale cover the whole K instead of one block of 64, so with int4 weights K past
// ~4.7k is already past it. Past it the conversion does not round, it returns a
// wrong number while the kernel still reports success. So split each lane into its
// high and low 16 bits, convert both with the magic number (each is inside its
// domain by construction: |hi| <= 32768, 0 <= lo <= 65535), and recombine as
// `hi * 65536 + lo`. The multiply is an exact power of two and the fp32 add is the
// only rounding, so the result is the correctly rounded fp32 of the original int32
// for every int32; and for |x| <= 2^22 it is bit for bit the value the magic number
// returned alone.
